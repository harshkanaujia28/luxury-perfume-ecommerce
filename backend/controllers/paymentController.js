...
export const verifyPayment = async (req, res) => {
  try {
    ...
    const productsWithDetails = await Promise.all(orderDetails.products.map(async item => {
    ...
    // ✅ Final Total
    const finalTotal = parseFloat((taxableAmount + taxAmount + (orderDetails.deliveryFee || 0)).toFixed(2));
